...
#ifndef BASE256_H
#define BASE256_H
atom_t*u64_digits_to_b256(const uint64_t value,uint16_t*const len,const bool little_endian){if(!value||NULL==len){return zalloc(1,atom_t);}atom_t*const result=zalloc(count_b256_digits_u64(value),atom_t);uint16_t i=0;for(uint64_t tempr=value;tempr;i++){result[i]=(atom_t)(tempr%B256_HIGH);tempr=(uint64_t)floorl(tempr/B256_HIGH);if(!tempr){break;}}*len=(uint16_t)(i+1);if(little_endian){atom_t*const reversed=array_reverse(result,*len);free(result);return reversed;}else{return result;}}atom_t*ldbl_digits_to_b256(const char*const ldbl_digits,uint16_t*const len,uint16_t*const int_len){const uint16_t pre_dec=(uint16_t)strcspn(ldbl_digits,".");char*const int_part=strndup(ldbl_digits,pre_dec),*const flot_part=str_reverse(ldbl_digits+pre_dec+1);const uint64_t lhs=strtoull(int_part,NULL,B10_HIGH),rhs=strtoull(flot_part,NULL,B10_HIGH);free(int_part),free(flot_part);uint16_t lhs_len,rhs_len;atom_t*const lhs_b256=u64_digits_to_b256(lhs,&lhs_len,true),*const rhs_b256=u64_digits_to_b256(rhs,&rhs_len,false);*len=(uint16_t)(lhs_len+rhs_len);*int_len=lhs_len;atom_t*const final_concat=array_concat(lhs_b256,rhs_b256,lhs_len,rhs_len);free(lhs_b256),free(rhs_b256);return final_concat;}uint64_t b256_to_u64_digits(const atom_t*const digits,const uint16_t len){if(!len){return 0;}uint64_t result=0;for(uint16_t i=len;i!=0;i--){result+=digits[i]*((uint64_t)powl(256,i));}return result;}char*b256_to_ldbl_digits(const atom_t*const digits,const uint16_t len,const uint16_t int_len,uint16_t*const out_int_len){if(!len){return make_empty_string();}const uint16_t flot_len=(uint16_t)(len-int_len);atom_t*const int_part=memcpy(alloc(int_len,atom_t),digits,sz(int_len,atom_t)),*const flot_part=memcpy(alloc(len-int_len,atom_t),digits+int_len,sz(len-int_len,atom_t));const uint64_t int_val=b256_to_u64_digits(int_part,int_len),flot_val=b256_to_u64_digits(flot_part,flot_len);free(int_part),free(flot_part);const uint16_t int_len10=count_digits_u64(int_val),flot_len10=count_digits_u64(flot_val);char*const flot_str=alloc(flot_len10,atom_t),*const finalval=alloc(int_len10+flot_len10+1,atom_t);snprintf(flot_str,(uint16_t)(flot_len10+1),"%" PRIu64 "",flot_val);char*const flot_str_be=str_reverse(flot_str);free(flot_str);snprintf(finalval,(uint16_t)(int_len10+flot_len10+2),"%" PRIu64 ".%s",int_val,flot_str_be);*out_int_len=count_digits_u64(int_val);return finalval;}
#endif 
#ifndef BNA_H
#define BNA_H
static atom_t*impl_to_digit_array_ldbl(const ldbl_t ldbl,const atom_t metadata,const atom_t flags);static atom_t*impl_to_digit_array_u64(const uint64_t u64,const atom_t metadata,const atom_t flags);static atom_t*make_array_header(const atom_t metadata,const uint16_t int_digits,const uint16_t flot_digits,const atom_t flags){const atom_t hdrlen=meta_header_offset(metadata);atom_t*const header=zalloc(hdrlen,atom_t);header[0]=metadata;header[hdrlen-1]=flags;if(meta_is_big(metadata)){atom_t lens[]={0,0,0,0};samb_u16_to_twoba(int_digits,lens+0,lens+1);samb_u16_to_twoba(flot_digits,lens+2,lens+3);memcpy(header+1,&lens,sz(4,atom_t));}else{header[1]=(atom_t)int_digits;header[2]=(atom_t)flot_digits;}return header;}atom_t*to_digit_array(const ldbl_t ldbl_in,const uint64_t u64,const atom_t value_flags,const atom_t metadata){const atom_t flags=ldbl_in<0 ? value_flags|FL_SIGN : value_flags;const ldbl_t ldbl=ldbl_in<0 ? fabsl(ldbl_in): ldbl_in;if(!compare_eps(ldbl,0.f,1e-11)){return impl_to_digit_array_ldbl(ldbl,metadata,flags);}else if(0!=u64){return impl_to_digit_array_u64(u64,metadata,flags);}else{return make_array_header(metadata,0,0,flags);}return NULL;}static atom_t*impl_to_digit_array_ldbl(const ldbl_t ldbl,const atom_t metadata,const atom_t flags){const bool is_base256=meta_is_base256(metadata),is_big=meta_is_big(metadata);const atom_t hdrlen=meta_header_offset(metadata);const uint32_t sigfigs=is_big ? MAX_SIGFIGS_BIG : MAX_SIGFIGS;char*const fullstr=alloc(sigfigs+3,char);snprintf(fullstr,sigfigs+2,"%Lf",ldbl);const atom_t
#ifdef PREFER_CHAR_CONV
nint_digits=(atom_t)strcspn(fullstr,"."),
#else 
nint_digits=count_digits_u64((uint64_t)floorl(ldbl)),
#endif 
nflot_digits=count_frac_digits(fullstr);printf("digits:%d%d\n",nint_digits,nflot_digits);atom_t*bn_tlated=alloc(nint_digits+nflot_digits+hdrlen,atom_t),*const init=make_array_header(metadata,nint_digits,nflot_digits,flags);printf("!!!DBGPRN%s\n",fullstr);memcpy(bn_tlated,init,sz(hdrlen,atom_t));if(is_base256){}else{
#ifdef PREFER_CHAR_CONV
