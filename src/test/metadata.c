...
Test(metadata, basic) {
  ...
  cr_assert_arr_eq(m, e, sz(atom_t, HEADER_OFFSET_BIG));
  free(m);

  m = make_array_header(TYP_NONE, 6, 7, FL_SIGN);
  atom_t f[HEADER_OFFSET] = { TYP_NONE, 6, 7, FL_SIGN };

  cr_assert_arr_eq(m, f, sz(atom_t, HEADER_OFFSET));
  free(m);
}
