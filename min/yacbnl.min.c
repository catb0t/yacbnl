...
char*const fullstr=alloc(char,sigfigs+3);
snprintf(fullstr,sigfigs+2,"%LG",ldbl);
const atom_t
...
char*const str=alloc(char,ndigits+2);
...
*const flot_part=memcpy(alloc(atom_t,flot_len),digits+int_len,sz(atom_t,flot_len));
...
snprintf(finalval,(uint16_t)(int_len10+flot_len10+2),"%" PRIu64 ".%s",int_val,flot_str_be);
...
void samb_u16_to_twoba(const uint16_t n,atom_t*const ah,atom_t*const al){
*ah=(atom_t)(n>>(atom_t)8);
*al=(atom_t)(n&(atom_t)0xFF);
}
